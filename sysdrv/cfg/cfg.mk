...
CONFIG_SYSDRV_PARTITION := 32K(env),512K@32K(idblock),4M(uboot),32M(boot),2G(rootfs)
...
CONFIG_SYSDRV_PARTITION := 64K(env),128K@64K(idblock),128K(uboot),2M(boot),2M(rootfs)
...
ifeq ($(CONFIG_SYSDRV_PARTITION),)
CONFIG_SYSDRV_PARTITION := 256K(env),256K@256K(idblock),1M(uboot),8M(boot),64M(rootfs)
##,32M(userdata),-(media)
endif
